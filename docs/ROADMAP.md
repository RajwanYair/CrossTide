...
### Domain Expansion (S501–S550) — v2.17.0–v2.20.0
- **OrderRoutingPreference** — algo/venue routing strategy with `isAlgorithmic`, `isTightSlippage` (S501)
- **SlippageEstimate** — fill-cost analysis with `totalCostBps`, `isHighSlippage` (S502)
- **ExecutionVenueConfig** — venue type + maker rebate config (S503)
- **DarkPoolIndicator** — dark-pool activity ratio + `isHighDarkActivity` (≥40%) (S504)
- **TickSizeRule** — tick-size band with `appliesTo(price)`, `appliesToPennies` (S505)
- **OrderBookImbalance** — bid/ask imbalance ratio with `isStrongImbalance` (S506)
- **LatencyArbitrageFlag** — venue latency divergence flag with `isSignificant` (≥5bps) (S507)
- **FillQualityReport** — fill rate, slippage, `isGoodFill` (≤5bps), `isFullFill` (S508)
- **CircuitBreakerStatus** — circuit-breaker level + halt metadata (S509)
- **CrossListingEntry** — multi-exchange cross-listing with ADR flag (S510)
- **CustodyAccountSummary** — custody account value + cash composition (S511)
- **SettlementCycleConfig** — T+N settlement cycle with `isStandardUS` guard (S512)
- **ClearingHouseMargin** — initial/maintenance margin requirements (S513)
- **TradeConfirmationRecord** — post-trade confirmation with fees, tax, commissions (S514)
- **MarketMicrostructureSnapshot** — spread/depth snapshot with `isTightSpread` (≤5bps) (S515)
- **ComplianceRuleViolation** — severity-scored compliance breach (S516)
- **WashSaleDetection** — 30-day window wash-sale IRS rule check (S517)
- **PositionLimitBreach** — position cap overage with `isMajorBreach` (≥20%) (S518)
- **StressTestScenario** — stress shocks with `isSevere` (≤-20%), `isHighVolatility` (S519)
- **RiskFactorExposure** — factor beta/loading with `isDominantFactor` (≥30%) (S520)
- **RegulatoryReportConfig** — filing frequency + deadline config (S521)
- **AmlFlagRecord** — AML severity enum + open-high-flag guard (S522)
- **ConcentrationRiskAlert** — single-asset over-weight alert (S523)
- **CounterpartyRiskScore** — PD-based grade enum + `isInvestmentGrade` (S524)
- **LeverageUtilization** — gross leverage + margin call risk (S525)
- **DrawdownRecoveryPlan** — recovery path with `isDeepDrawdown` (≤-20%) (S526)
- **AuditTrailHash** — cryptographic hash chain for audit integrity (S527)
- **PreTradeCheckResult** — pass/warn/block pre-trade compliance check (S528)
- **EsgScoreSnapshot** — E/S/G composite scores with `isHighEsg` (≥70) (S529)
- **CarbonExposureEstimate** — Scope 1/2/3 CO₂e estimate (S530)
- **ReturnAttributionResult** — Brinson allocation/selection decomposition (S531)
- **FactorLoadingSnapshot** — multi-factor regression loadings (S532)
- **YieldCurveSnapshot** — 2Y/5Y/10Y/30Y rates with `isInverted`, `isFlat` (S533)
- **CreditSpreadSnapshot** — IG/HY/EM spread levels with widening/narrowing guards (S534)
- **MacroRegimeIndicator** — Goldilocks/Stagflation/Deflation/Recession phases (S535)
- **CarryTradeSignal** — interest-rate carry opportunity with `isHighCarry` (≥5%) (S536)
- **MomentumFactorSignal** — percentile rank + 12M returns with `isTopDecile` (S537)
- **ValueFactorSignal** — value percentile + earnings yield with `isCheap` (≥80%) (S538)
- **QualityFactorSignal** — ROE/debt ratio quality with `isHighQuality` (≥80%) (S539)
- **SizeFactorSignal** — market cap bucket with SMB tilt detection (S540)
- **VolatilityFactorSignal** — low-vol rank + HV with `isDefensive` (beta≤0.7) (S541)
- **DividendGrowthEstimate** — 5Y CAGR + payout sustainability (S542)
- **FreeCashFlowYield** — FCF yield + EV/FCF cheapness screen (S543)
- **EnterpriseValueEstimate** — EV/EBITDA with `isCheap` (≤12×) / `isExpensive` (≥25×) (S544)
- **IntrinsicValueEstimate** — DCF/Graham blend with `marginOfSafetyPercent` (S545)
- **AppUpdateManifest** — OTA update manifest with `isUpdateAvailable`, `isMandatory` (S546)
- **RemoteConfigSnapshot** — runtime key-value config with `isFresh`, `isFallback` (S547)
- **CrashReportSummary** — crash analytics with `isStable` (≥99.5%), `hasCriticalInstability` (S548)
- **AbTestAssignment** — A/B variant assignment with `isControl`/`isTreatment` (S549)
- **UserCohortDefinition** — user segmentation cohort with declarative `filterExpression` (S550)
- **~1900+ passing tests**, 0 analyze issues | v2.20.0+34

### Domain Expansion (S551–S600) — v2.21.0–v2.24.0
- **SignalFunnelSummary** — signal detection funnel stages (detected/evaluated/triggered/sent) (S551)
- **SignalGeneratorConfig** — composite signal generator with `MethodWeightEntry` per-method weighting (S552)
- **SignalDivergenceAlert** — price/indicator divergence with `DivergenceDirection` enum (S553)
- **AppHealthMonitor** — runtime health events with `HealthEvent` + `HealthEventSeverity` (S554)
- **DataLineageRecord** — data provenance tracking with `DataQualityTier` enum (S555)
- **NetworkQualitySnapshot** — latency/jitter measurement with `NetworkQualityRating` (S556)
- **BacktestSensitivityResult** — parameter sensitivity grid with `SensitivityAxis` + `SensitivityCell` (S557)
- **CacheWarmupConfig** — cache pre-warming config with `WarmupStrategy` enum (S558)
- **PriceGapAnalysis** — gap classification with `PriceGapType` (common/breakaway/runaway/exhaustion) (S559)
- **PortfolioHealthScore** — 0–100 composite score with `PortfolioHealthComponent` (S560)
- **RebalanceExecution** — rebalance trade plan with `RebalanceLeg` + `RebalanceLegStatus` (S561)
- **TechnicalAlertSummary** — roll-up of active technical alerts per ticker (S562)
- **EarningsSurpriseRecord** — EPS actual vs consensus with `EarningsSurpriseDirection` (S563)
- **AnalystRatingChange** — broker upgrade/downgrade with `RatingChangeDirection` + `AnalystRatingTier` (S564)
- **IndexRebalanceEvent** — index reconstitution events with `IndexRebalanceType` (S565)
- **ShortInterestSnapshot** — short % float + days-to-cover snapshot (S566)
- **DividendCutAlert** — dividend change alerts with `DividendChangeType` enum (S567)
- **StockSplitEvent** — stock split details with `StockSplitType` (forward/reverse) (S568)
- **OptionsExpiryDate** — options expiry schedule with `OptionsExpiryStyle` enum (S569)
- **ImpliedMoveEstimate** — options-implied earnings move estimate (S570)
- **InstitutionalOwnershipEntry** — 13F ownership change with `OwnershipChangeDirection` (S571)
- **SecFilingEntry** — SEC EDGAR filing metadata with `SecFilingType` enum (S572)
- **RegimeSwitchAlert** — macro regime transition alert (S573)
- **CrossAssetCorrelation** — equity/bond/commodity correlations with `CorrelationAssetClass` (S574)
- **MarketImpactModel** — Almgren-Chriss/square-root/Kyle impact with `MarketImpactModelType` (S575)
- **DataIngestionEvent** — single data ingestion event log (S576)
- **AlertChannelMetrics** — delivery channel aggregate metrics (S577)
- **SectorRotationSignal** — cross-sector capital rotation with `SectorRotationDirection` (S578)
- **ContextAwareSignal** — enriched signal with `SignalContextSource` (S579)
- **DataFreshnessPolicy** — data freshness enforcement with `StaleDataPolicy` (S580)
- **BacktestParameterGrid** — multi-dimensional grid search with `GridAxis` + `GridCell` (S581)
- **PortfolioTurnoverMetric** — annualised portfolio turnover and average holding days (S582)
- **MarketLiquidityRating** — composite liquidity score with `LiquidityRatingGrade` (S583)
- **VolatilityRegimeAlert** — HV regime transition with `VolatilityRegimeTransition` enum (S584)
- **NewsEventEntry** — structured news event with `NewsEventSentiment` (S585)
- **TradeIdeaRecord** — trade thesis with `TradeIdeaConviction` + `TradeIdeaDirection` (S586)
- **UserAlertPreference** — per-user alert delivery preferences (S587)
- **WatchlistHealthCheck** — watchlist data freshness + validity with `WatchlistHealthStatus` (S588)
- **PlatformBuildInfo** — OS/Flutter/Dart version snapshot (S589)
- **QuoteQualityMetric** — quote freshness, delay, and spread quality (S590)
- **RealTimeSessionConfig** — WebSocket/SSE session config with `RealTimeTransport` (S591)
- **TickerLifecycleEvent** — listing/delisting/split/rename with `TickerLifecycleEventType` (S592)
- **MarketCapTierClassifier** — market cap tier with `CapSizeTier` enum (S593)
- **HeatMapColorScale** — heat-map color gradient with `ColorStop` value/hex pairs (S594)
- **FeatureFlagOverride** — runtime flag override with auditor + timestamp (S595)
- **OnboardingChecklistItem** — first-run checklist with `OnboardingItemStatus` (S596)
- **QuickActionConfig** — home-screen quick action with `QuickActionPlatform` (S597)
- **WidgetRefreshPolicy** — home-screen widget refresh with `WidgetRefreshTrigger` (S598)
- **PlatformPerformanceBudget** — frame-time and jank detection thresholds (S599)
- **AppLocaleOverride** — per-user locale override (S600)
- **~3175+ passing tests**, 0 analyze issues | barrel: **520 exports** | v2.24.0+38
