...
> **656 test files / 7371 tests / 0 failures** (commit `2a99fad`)

## [Unreleased]

### 🔒 Security

- **Signal DSL, portfolio rebalance, and WebSocket fan-out now enforce abuse-control limits** (roadmap S05): `signal-dsl` expressions/scripts had no length cap (only script `loop iterations` were bounded), `portfolio-rebalance` accepted unbounded `holdings`/`targets` arrays, and the `TickerFanout` Durable Object accepted unlimited concurrent WebSocket subscribers per ticker. Added a 2,000-char expression cap, 10,000-char script cap, 500-item holdings/targets caps (all `413 Payload Too Large`), and a 1,000-connection cap per ticker (`503`), each with a focused regression test. `screener` and `news/sentiment` already enforced a 50-item cap and were left unchanged.

### ✨ Added

- **Portfolio rebalance and position/risk explainability** (roadmap D08): `calculateRebalance` now returns a `RebalanceExplanation` (drift threshold, largest-drift ticker, holdings with no target allocation, limitations), and `computePositionRisk`/`computePortfolioHeat` return `PositionRiskExplanation`/`PortfolioHeatExplanation` (whether a stop/target was supplied, excluded positions and why, largest risk contributor) — completing the explainability pattern already shipped for consensus, backtest, and screener outputs. 5 new tests.
- **Self-service alert history export and deletion, plus an enforced retention purge** (roadmap S04): `GET /api/alerts/history/export` (JSON or `?format=csv`) and `DELETE /api/alerts/history` close the gap `docs/DATA_RETENTION.md` had explicitly tracked as "no self-service export/delete endpoint". A new daily Cloudflare Cron Trigger (`0 3 * * *`) calls `purgeExpiredAlertHistory()`, closing the matching "no scheduled purge job" gap and enforcing the documented 180-day retention window automatically instead of depending on an operator-run query. While editing `worker/wrangler.toml`'s `[triggers]`, a second, unrelated bug was found and fixed: the `scheduled()` handler already branched on `"0 0 * * *"` for R2 archival (roadmap Q15, documented as "cron-driven"), but that cron string was never registered, so nightly archival never actually ran. Adds `worker/migrations/0005_alert_history_purge_index.sql` (neither existing composite index leads with `fired_at`, so the bare purge query needed its own). 12 new tests.
- **MCP server bearer-token authentication** (roadmap E04): `mcp-server/src/index.ts`'s HTTP calls are extracted into a new `mcp-server/src/api-client.ts` (independently unit-testable, unlike the original file, which called `main()`/`server.connect()` unconditionally at module load) and now forward an optional `CROSSTIDE_API_TOKEN` env var as `Authorization: Bearer <token>` on every request, for deployments where `CROSSTIDE_API_URL` points somewhere other than localhost. 6 new tests.
- **MCP server health check in CI** (roadmap P06): `smoke.yml` probes shell, quote, chart, search, and WebSocket over HTTP, but the MCP server has no production HTTP endpoint (local stdio only) and no CI step even built that workspace — a broken build would only surface if someone ran it by hand. Added `scripts/check-mcp-health.mjs` (spawns the built server, lists its tools over the real MCP SDK `Client`/`StdioClientTransport`, and asserts the tool set), wired as `npm run check:mcp-health` into `ci.yml` — verified to actually fail on an injected tool-name mismatch before being trusted. This is also the first test coverage of any kind for `mcp-server/src/index.ts`'s server wiring (tool registration, request handlers), which unit tests cannot reach.
- **Reproducible clean-install verification in CI** (roadmap S06): SBOM generation, `npm audit signatures`, and dependency review already ran in CI, but nothing verified that a clean `npm ci` reproduces the exact tree `package-lock.json` claims to pin rather than silently rewriting it. Verified empirically first (1438 packages installed, lockfile SHA-256 identical before and after), then added a "Verify reproducible install" step to `ci.yml` that asserts `git diff --exit-code -- package-lock.json` after every install, on every run.
- **Metrics Purpose Catalog** (roadmap G05): `docs/DATA_RETENTION.md` now names why each existing adoption event and reliability signal is collected. Investigating "documentation success" found `docs-site` carries no analytics at all, and that its only real public deployment is GitHub Pages (`pages.yml`) — `cf-pages.yml` never builds `docs-site`, despite implying Cloudflare Pages is primary. Documented rather than implemented: adding a page-view metric needs the GitHub Pages domain registered with Plausible first, an operator action outside this repository.
- **Widget shadow-DOM accessibility** (roadmap E03): the chart, quote, and consensus custom elements' loading state now uses `role="status"`/`aria-live="polite"` (matching the existing `role="alert"` on errors), and the quote/consensus render output uses `role="group"` with an `aria-label` summarizing the current value plus `aria-live="polite"` so value updates are announced without moving focus. Closed shadow DOM only blocks JS access via `element.shadowRoot`, not the accessibility tree, so this reaches real assistive technology despite being unreachable from unit tests — documented in `docs-site/src/content/docs/widgets.mdx`.
- **`docs/adr/0022-webllm-local-embeddings-signal-dsl-assistance.md`** (roadmap R01): evaluates WebLLM, local embeddings, and Signal DSL assistance against `ai-disclaimer.ts`'s existing feature boundary. Decides against implementing WebLLM/local embeddings now (no repository build bundles either, and both would need real browser-measured bundle/latency/memory evidence this session could not fabricate honestly), recommends non-AI grammar-driven Signal DSL assistance as a first, ungated deliverable, and records the evidence bar a future implementation ADR must clear.
- **`npm run resolve-blockers`** (roadmap ops tooling): an interactive CLI that walks every credential and cloud-provisioning gap recorded in `docs/ROADMAP.md`'s "External Blockers" section — Cloudflare `wrangler` auth, KV/D1/R2 provisioning, GitHub Actions secrets (`CLOUDFLARE_API_TOKEN`, `CLOUDFLARE_ACCOUNT_ID`, `NPM_TOKEN`), npm publish auth, optional provider API keys, and Fly.io monitoring auth — checks each with a read-only command, and only with explicit per-item confirmation runs the official vendor CLI (`wrangler`/`npm`/`gh`/`flyctl`) with the terminal handed over via `stdio: "inherit"`, so this script never reads, stores, or transmits a secret itself. Items with no CLI-automatable fix (recruiting real users for T06/U01, publishing a live demo for G01, sourcing a futures data vendor for D01) are reported as "Manual — not automatable" with a pointer to the doc that owns the decision. `worker/wrangler.toml` is a Worker project distinct from the repo-root Pages `wrangler.toml`, so every wrangler invocation runs scoped to `worker/`; error detail lines surface the CLI's actual `[ERROR]` output instead of a guessed cause.
- **`docs/API_VERSIONING.md`** (roadmap E01): records the real per-surface version state — OpenAPI `info.version` is static and never bumped, the Worker envelope `schemaVersion` is versioned independently, MCP tools version at the server level only, widgets now have `WIDGET_CONTRACT_VERSION` (see below), and `packages/domain/package.json` is **not** in the release skill's version-bump table despite `docs/PACKAGE_CONTRACTS.md` previously calling it a "versioned package contract" — that table's stability column now says so accurately.
- **MCP server now has per-tool rate limiting and documented auth/error contracts** (roadmap E04): added `mcp-server/src/rate-limit.ts`, a 30-calls-per-minute token bucket keyed by tool name, as defense-in-depth ahead of the Worker's own IP-based limiter — a looping agent can no longer hammer a tool indefinitely before the underlying HTTP call is even made. `mcp-server/README.md` now documents the stdio trust boundary, the lack of bearer-token auth for non-localhost `CROSSTIDE_API_URL` use, and the standard MCP error-envelope shape every tool failure already returned. 4 new tests.
- **Widgets now expose an explicit contract version** (roadmap E03): `CrosstideChartElement`, `CrosstideQuoteElement`, and `CrosstideConsensusElement` had no way for a host page to check compatibility before this. Added `WIDGET_CONTRACT_VERSION`, a static `.version` property on each class, and a `data-widget-version` DOM attribute set on connect. `docs-site/src/content/docs/widgets.mdx` now documents the version contract, the actual attribute-only theming model (closed Shadow DOM has no `--ct-*` CSS custom-property override surface), and the CSP directives a self-hosted deployment needs.
- **`docs/DEPRECATION_POLICY.md`** (roadmap E06): defines the deprecation signal, minimum removal-horizon, and migration-note template for Worker routes, providers, domain indicators, published packages, widgets, and config surfaces.
- **`docs/DATA_RETENTION.md`** (roadmap S04): documents what CrossTide stores in D1 (passkeys, BYOK keys, alert history) and in analytics/error-reporting sinks, current retention, consent model, and the known gaps (no scheduled `alert_history` purge job, no self-service export/delete endpoint).
- **Settings now has an analytics opt-out toggle** (roadmap G05, S04): `AnalyticsClient.setEnabled(false)` existed but was never wired to a UI control. Added `src/core/telemetry-preference.ts` (persists the choice in `localStorage`, applied at boot before the first event fires) and a "Anonymous analytics" toggle in Settings, with tests covering render, persistence, and reload.
- **`docs/USER_FEEDBACK_PLAN.md`** (roadmap T06): defines the recruitment, consent, session protocol, and findings format for the five-journey external usability evidence the roadmap requires; no round has been run yet.
- **ADR-0020 and ADR-0021** (roadmap R03, R04): record the evaluation criteria for advanced research indicators/ML/alternative-data proposals and for reassessing framework, rendering, storage, or hosting choices, so future proposals are checked against a written bar instead of ad hoc judgment.
- **Four new issue templates** (roadmap G02): `data_quality.yml`, `provider_outage.yml`, `security_hardening.yml` (explicitly routes real vulnerabilities to private advisory reporting), and `adr_proposal.yml`, alongside the existing bug/feature/API templates.
- **`npm run check:indicator-docs`** (roadmap E05): `scripts/generate-indicator-docs.ts` existed but was never wired into an automated gate, so `docs/INDICATORS.md` had silently drifted (217→218 modules, missing the `validate-ohlcv` module and the new `BacktestExplanation`/`FundamentalFilterExplanation` types). The new check regenerates and diffs against the committed file and is now part of `check:repo-contracts`.
- **Smoke test now covers the app shell and the WebSocket fan-out route** (roadmap P06): `smoke.yml` previously checked health/quote/chart/search/FRED only. Added an app-shell root-element check and a WebSocket-route reachability check (expects `426 Upgrade Required` for a non-upgrade request, since a full WebSocket handshake isn't practical from a shell script).
- **Release cadence, support horizon, and coordinated-disclosure timeline documented in `.github/SECURITY.md`**, and a "Good First Issues And Review Expectations" section plus changelog-in-the-same-PR guidance added to `.github/CONTRIBUTING.md` (roadmap G03, G04).
...
- **OHLCV gap detection is now exchange-calendar aware** (roadmap D06): a plain calendar-day gap threshold treats every Friday-to-Monday transition as a 3-day gap, so a strict threshold flagged ordinary weekends as data-quality issues. `validateOhlcv` now accepts an optional `exchange` and, when given, counts only missed `trading` days on that exchange's calendar — weekend-only gaps are no longer misflagged, while a gap that actually skips a trading session is still caught.
- **Screener fundamental filters are now explainable** (roadmap D08): `matchesFundamentalFilters` only ever returned a boolean, so a user could not see why a ticker was included or excluded from screener results. The new `explainFundamentalFilters` reports which supplied constraints matched, failed, or were skipped due to missing data, without changing the existing boolean function's hot-path behavior or signature.
...
- **Native app packaging removed — CrossTide is web-only** (roadmap F05): `src/core/tauri-bridge.ts` was fully dead code — no `@tauri-apps` dependency existed anywhere in the repository to back its `__TAURI__` global detection, so it could never have run. Deleted it and its test. Capacitor native iOS/Android packaging is removed entirely per an explicit product decision: `capacitor.config.ts`, all five `@capacitor/*` dependencies, the `cap:sync`/`cap:android`/`cap:ios` npm scripts, and `docs/CAPACITOR_SUPPORT.md` are gone, and `README.md`, `docs/CAPABILITY_MATRIX.md`, `docs/OWNERSHIP.md`, and `docs/OPERATIONS.md` no longer reference them.

### 🔄 Changed

- **Nine devDependency version pins across three workspaces resolved to package versions that were never published.** A prior automated dependency-update merge set `@biomejs/biome` to `2.5.8`, `@playwright/mcp` to `0.0.79`, `postcss` to `8.5.26`, and `@types/node` (mcp-server) to `26.2.0` — none of which exist on the npm registry, confirmed by repeated fresh `E404` responses even after a full npm cache verify/garbage-collect (684 MB of stale cache content) and a complete `node_modules` + lockfile rebuild from zero. All four are corrected to their real latest published version. `valibot` (root + mcp-server), `@astrojs/markdown-remark`, `unist-util-visit`, and all six `workbox-*` packages were genuinely behind and are now bumped to their real latest versions. A clean install now resolves 1472 packages with zero registry errors, and the dependency audit finding count dropped from 21 to 2 as a side effect (most of the noise was transitive exposure through the phantom-version packages). `npm run ci` passes end-to-end with zero errors and zero warnings against the freshly verified dependency tree.
- **The supply-chain audit gate now distinguishes explained from unexplained findings** (roadmap S01): `npm run check:supply-chain` previously failed outright on any high/critical `npm audit` finding, with no path to green other than a breaking upstream dependency bump. The remaining findings trace back to root advisories (currently just `js-yaml`) that have no published fix yet and are not reachable from the shipped browser bundle or Worker runtime. `scripts/audit-dependencies.mjs` now requires each advisory to be covered by a dated, owned, mitigation-documented exception entry (expiring in 90 days), traces multi-hop dependency chains back to an acknowledged root, and only reports the exceptions actually in use for the current finding set rather than the full registered list — so a genuinely new or unrelated finding still fails the gate immediately, and a no-longer-triggered exception becomes visible instead of silently accumulating.
- **Four GitHub Actions workflows called raw `npm audit` directly, bypassing the audit exception mechanism entirely.** `ci.yml`, `release.yml`, `security.yml`, and `supply-chain.yml` each ran `npm audit --omit=dev --audit-level=high` inline instead of `npm run audit:dependencies`, so a real CI run would still fail on the same findings the local wrapper had just resolved. All four now call the wrapper script. `release.yml` also duplicated the architecture, contrast, and audit checks that `npm run ci` (its own first step) already runs; the four redundant steps were removed.
- **Roadmap sprint status updated:** the next 25 planned roadmap items were audited against the real repository state and worked in delivery order in `docs/ROADMAP.md`; `docs/SPRINT-50.md` was merged into `docs/ROADMAP.md` for a single roadmap/sprint source of truth.
...
- **The Settings route rendered a years-old static HTML fallback instead of the real Settings card.** `main.ts`'s `cardContainers` map (which drives lazy `CardModule` mounting) omitted `"settings"`, so `src/cards/settings.ts`/`settings-card.ts` — including locale, notifications, method weights, per-card settings, layout presets, shortcuts, and this session's own telemetry opt-out toggle — were registered but never mounted. Live browser verification showed the actual rendered page still had the old two-option theme select and no contrast/telemetry/locale controls at all. `settingsCard`'s `onExport`/`onImport`/`onClearWatchlist`/`onClearCache` were also no-op stubs (`/* wired by main */`) — the real logic lived in ~150 lines of `main.ts` code hand-wired to static element IDs instead. Fixed by: adding `settings` to `cardContainers`; porting the real export/import/clear/full-backup/image-export logic into `settingsCard`'s callbacks; adding the previously-missing color-blind palette select, image export button, full JSON/CSV export buttons, and PWA install prompt to `settings.ts`; sharing one `PwaInstallManager` singleton (`getPwaInstallManager()`, new `offReady`/`offInstalled` methods) between `main.ts` and the card; and dispatching a `crosstide:config-changed` event so `main.ts`'s own watchlist rendering and refresh-interval timer stay in sync with config changes the Settings card now makes independently. `index.html`'s static settings markup is replaced with the same loading placeholder every other lazy card uses. Also fixed three tests in `tests/unit/cards/settings-coverage.test.ts` that silently passed without testing anything (`if (!btn) return`) because they queried legacy element IDs that had never existed in `settings.ts`'s actual output.
- **The Speculation Rules API (route prefetching) violated the app's own CSP on every page load**, logging a console error and never actually prefetching anything: `injectSpeculationRules` set inline `<script type="speculationrules">` content, which the strict `script-src` (no `'unsafe-inline'`) blocks outright. Verified live in a browser. Rewrote the injector to use a `blob:` URL (`script-src ... blob:`, added consistently across all five independent CSP definitions — `src/core/csp-builder.ts`, `scripts/gen-csp.mjs`/`public/_headers`, `worker/security.ts`, `vite.config.ts`'s dev/preview headers, and `index.html`'s meta tag), which is spec-correct and CSP-clean. Current Chrome does not yet support external `src` for speculation rules ("External speculation rules are not yet supported"), so `main.ts` now always uses the existing, universally-supported `<link rel="prefetch">` fallback; `injectSpeculationRules` keeps the CSP-safe implementation ready for when browser support lands.
