...
- **OpenAPI backlog burn-down advanced by another 10 routes** (roadmap E2): `worker/routes/openapi.ts` now documents `GET /api/compare`, `GET /api/indicators`, `GET /api/economic`, `GET /api/movers`, `GET /api/sector-heatmap`, `GET /api/news`, `GET /api/fred`, `GET /api/crypto/search`, `GET /api/crypto/{id}/chart` and `GET /api/og`. `tests/unit/worker/openapi-drift.test.ts` was updated to shrink `KNOWN_GAP`, and regenerated `src/core/api-types.ts` now exposes 29 typed route operations. Remaining undocumented routes in [#105](https://github.com/RajwanYair/CrossTide/issues/105): 19.
