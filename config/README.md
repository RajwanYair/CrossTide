...
- Keep root-level files that tools discover automatically or that define a project entry point at the repository root. This includes `biome.json`, `playwright.config.ts`, `tsconfig*.json`, `vite*.config.ts`, `vitest*.config.ts`, and `wrangler.toml`.
