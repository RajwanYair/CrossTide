...
import { evaluate } from "../../../src/domain/williams-r-method";
import { makeCandles } from "../../helpers/candle-factory";

describe("williams-r-method evaluate", () => {
  ...
  it("returns a valid signal for sufficient data", () => {
    ...
    expect(result!.method).toBe("WilliamsR");
    expect(["BUY", "SELL", "NEUTRAL"]).toContain(result!.direction);
  });

  it("populates all MethodSignal fields", () => {
    const closes = Array.from({ length: 20 }, (_, i) => 100 + i);
    const result = evaluate("NFLX", makeCandles(closes));
    expect(result!.ticker).toBe("NFLX");
    expect(result!.method).toBe("WilliamsR");
  });
});
