...
function sig(method: string, direction: "BUY" | "SELL" | "NEUTRAL"): MethodSignal {
  return { ticker: "AAPL", method: method as MethodSignal["method"], direction, description: `${direction} ${method}`, currentClose: 150, evaluatedAt: "2024-01-15" };
}
...
describe("evaluateAlerts", () => {
  it("fires BUY alert on first evaluation", () => {
    const state = createAlertState("AAPL");
    const { alerts, nextState } = evaluateAlerts(state, [sig("Micho", "BUY")], null, DEFAULT_ENABLED_ALERTS, "2024-01-15");
  ...
  it("does not fire same BUY alert twice", () => {
    const state = createAlertState("AAPL");
    const { nextState } = evaluateAlerts(state, [sig("Micho", "BUY")], null, DEFAULT_ENABLED_ALERTS, "2024-01-15");
    const { alerts } = evaluateAlerts(nextState, [sig("Micho", "BUY")], null, DEFAULT_ENABLED_ALERTS, "2024-01-16");
    expect(alerts.length).toBe(0);
  });

  it("resets BUY when SELL fires", () => {
    const state = createAlertState("AAPL");
    const { nextState: s1 } = evaluateAlerts(state, [sig("Micho", "BUY")], null, DEFAULT_ENABLED_ALERTS, "2024-01-15");
    const { nextState: s2 } = evaluateAlerts(s1, [sig("Micho", "SELL")], null, DEFAULT_ENABLED_ALERTS, "2024-01-16");
    ...
    const { alerts } = evaluateAlerts(s2, [sig("Micho", "BUY")], null, DEFAULT_ENABLED_ALERTS, "2024-01-17");
    expect(alerts.length).toBe(1);
  });

  it("NEUTRAL resets both sides", () => {
    const state = createAlertState("AAPL");
    const { nextState: s1 } = evaluateAlerts(state, [sig("RSI", "BUY")], null, DEFAULT_ENABLED_ALERTS, "2024-01-15");
    const { nextState: s2 } = evaluateAlerts(s1, [sig("RSI", "NEUTRAL")], null, DEFAULT_ENABLED_ALERTS, "2024-01-16");
  ...
  it("fires consensus BUY alert", () => {
    const consensus: ConsensusResult = { ticker: "AAPL", direction: "BUY", buyMethods: [], sellMethods: [], strength: 0.5 };
    const state = createAlertState("AAPL");
  ...
  it("skips alerts not in enabled set", () => {
    ...
    const { alerts } = evaluateAlerts(state, [sig("Micho", "BUY"), sig("RSI", "BUY")], null, enabled, "2024-01-15");
    expect(alerts.length).toBe(1);
  ...
  it("updates lastEvaluatedAt", () => {
    const state = createAlertState("AAPL");
    const { nextState } = evaluateAlerts(state, [], null, DEFAULT_ENABLED_ALERTS, "2024-01-15T10:00:00Z");
